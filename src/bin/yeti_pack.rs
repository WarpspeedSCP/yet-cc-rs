...
"#);
	...
	let current_folder = files
		.first()
		.with_context(|| "Expected the folder to contain files!")?
		.parent()
		.with_context(|| "Expected the folder to have a parent!")?;

	let yaml_folder = current_folder
		.parent()
		.with_context(|| "Expected the yaml folder to have a parent!")?
		.join(current_folder.file_name().with_context(|| "Expected folder to have a file name!")?.replace(".script", ".yaml"));

	do_archive_command(
		&yaml_folder,
		current_folder,
		&current_dir().join("sn.bin"),
		true,
		true
	)
}
