...
pub fn transmute_to_array<const SIZE: usize>(address: usize, input: &[u8]) -> Result<[u8; SIZE]> {
	input.get(address..address + SIZE).map_or_else(
		|| {
			Err(anyhow!(
				"Couldn't get array of size 0x{SIZE:08X} at 0x{address:08X}"
			))
		},
		|it| {
			it.try_into()
				.map_err(|_| anyhow!("Error getting array of size 0x{SIZE:08X} at 0x{address:08X}"))
		},
	)
}

pub fn transmute_to_u32(address: usize, input: &[u8]) -> Result<u32> {
	let array = transmute_to_array(address, input).map_err(|_| YetiError::ParseU32 { address })?;
	Ok(u32::from_le_bytes(array))
}

pub fn transmute_to_u16(address: usize, input: &[u8]) -> Result<u16, YetiError> {
	let array = transmute_to_array(address, input).map_err(|_| YetiError::ParseU16 { address })?;
	Ok(u16::from_le_bytes(array))
}

#[derive(Clone, Copy, Debug)]
pub enum YetiError {
	ParseU16 { address: usize },
	ParseU32 { address: usize },
	ParseHeader { address: usize, header_size: usize },
	ParseOpcode { address: usize, opcode: u8 },
}

impl Display for YetiError {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		match self {
			YetiError::ParseU16 { address } => {
				write!(f, "Could not read 2 bytes at 0x{address:08X}")
			}
			YetiError::ParseU32 { address } => {
				write!(f, "Could not read 4 bytes at 0x{address:08X}")
			}
			YetiError::ParseHeader {
				address,
				header_size,
			} => {
				write!(f, "Could not read {header_size} bytes at 0x{address:08X}")
			}
			YetiError::ParseOpcode { address, opcode } => {
				write!(
					f,
					"Opcode {opcode:02X} not recognised at address 0x{address:08X}"
				)
			}
		}
	}
...
impl SJISChar {
	fn new(value: char) -> Self {
		if value.is_ascii() {
			...
			let sjis_u16 = transmute_to_u16(0, &sjis).unwrap();
...
pub static ITALIC_MAPPING_TABLE: Lazy<HashMap<char, SJISChar>> = Lazy::new(|| {
	...
	let data = tinyjson::JsonValue::from_str(&file).unwrap();

	if let JsonValue::Object(obj) = data {
		for (k, v) in obj {
			map.insert(
				k.chars().next().unwrap(),
				SJISChar::from_number(
					u16::from_str_radix(&v.get().map(|it: &String| it.as_str()).unwrap_or(""), 16)
						.unwrap(),
				),
			);
		}
...
macro_rules! opcode_common_action {
	($self: ident, $op: ident, $action: block, $array_action: block) => {
		match $self {
			...
			Opcode::OP_03_DIRECT_JUMP_PHANTOM($op) => $action,
			Opcode::OP_04_JUMP_TO_SCRIPT_WITH_OFFSET_PHANTOM($op) => $action,
			...
			Opcode::OP_18_PHANTOM($op) => $action,
			...
			Opcode::OP_36_LP($op) => $action,
			Opcode::OP_37($op) => $action,
			Opcode::OP_3F_LP($op) => $action,
			...
			Opcode::OP_46_LP_CHARNAME($op) => $action,
			...
			Opcode::OP_50_PHANTOM($op) => $action,
			...
			Opcode::OP_7A_LP_B10($op) => $action,
			Opcode::OP_7B($op) => $action,
			Opcode::OP_7B_ROOT_XBOX($op) => $action,
			Opcode::OP_7D_LP($op) => $action,
			Opcode::OP_80_PHANTOM($op) => $action,
			...
			Opcode::OP_8C_PHANTOM($op) => $action,
			Opcode::OP_8C_XBOX($op) => $action,
			Opcode::OP_8D_XBOX($op) => $action,
			Opcode::OP_8E_ROOT_XBOX($op) => $action,
			Opcode::OP_8F_ROOT_XBOX($op) => $action,
			Opcode::OP_90_PHANTOM_CHARNAME($op) => $action,
			Opcode::OP_92_LP($op) => $action,
			Opcode::OP_93_LP($op) => $action,
			Opcode::OP_FF($op) => $action,
...
pub fn ends_with_ignore_case(a: &dyn AsRef<str>, b: &dyn AsRef<str>) -> bool {
	a.as_ref().ends_with(b.as_ref())
		|| a.as_ref()
			.to_ascii_uppercase()
			.ends_with(&b.as_ref().to_ascii_uppercase())
}

pub trait OkWrappable
where
	Self: Sized,
{
...
impl<U> OkWrappable for U where U: Sized {}
