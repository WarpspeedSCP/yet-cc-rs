...
macro_rules! opcode_common_action {
  ($self: ident, $op: ident, $action: block, $array_action: block) => {
    match $self {
      Opcode::OP_00($op) => $action,
      Opcode::OP_01($op) => $action,
      Opcode::OP_02($op) => $action,
      Opcode::OP_03($op) => $action,
      Opcode::OP_04($op) => $action,
      Opcode::OP_05($op) => $action,
      ...
      Opcode::OP_31($op) => $action,
      Opcode::OP_32($op) => $action,
      ...
      Opcode::OP_44($op) => $action,
      Opcode::OP_45($op) => $action,
      Opcode::OP_47($op) => $action,
      Opcode::OP_48($op) => $action,
      Opcode::OP_49($op) => $action,
      Opcode::OP_4A($op) => $action,
      ...
      Opcode::OP_85($op) => $action,
      Opcode::OP_86($op) => $action,
